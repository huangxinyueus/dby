...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        try:
            ...
            # 为验证页面的 Server Action 补充请求头
            headers_verify = self.getheader(-1)
            headers_verify['accept'] = 'text/x-component'
            headers_verify['content-type'] = 'text/plain;charset=UTF-8'
            verify = requests.post(f'{xurl}/search/{data["hash"]}',
                                   headers=headers_verify,
                                   data=b'[]',  # 按照 curl 结构，此处无需传复杂的参数
                                   )
            if verify.status_code == 200:
                eid = data['data'][0]['eid']
                rdata = json.dumps([{"eid": eid}], separators=(",", ":")).encode()

                # 为解析页面的 Server Action 补充请求头
                headers_go = self.getheader(1)
                headers_go['accept'] = 'text/x-component'
                headers_go['content-type'] = 'text/plain;charset=UTF-8'

                res = requests.post(f'{xurl}/go/{eid}', headers=headers_go, data=rdata)

                try:
                    purl = json.loads(res.text.strip().split('\n')[-1].split(":", 1)[-1])['data']['link']
                except Exception as e:
                    print("解析播放链接失败，返回内容：", res.text)
                    raise e
                ...
                raise Exception(f'验证失败，状态码: {verify.status_code}')
            ...
            requests.post("http://127.0.0.1:9978/action", data=params, headers={
    ...
    def getsign(self, key, pg):
        ...
        data = json.dumps([{"cat": "all", "query": key, "pageNum": int(pg), "enableSearchMusic": False,
                            "enableSearchGame": False, "enableSearchEbook": False}], separators=(",", ":"),
                          ensure_ascii=False).encode('utf-8')
        res = requests.post(f"{xurl}/", headers=headers, data=data).text
        try:
            hash_val = re.search(r'"hash"[,\:]\s*"([^"]+)"', res).group(1)
            sign_val = re.search(r'"sign"[,\:]\s*"([^"]+)"', res).group(1)
            sha_val = re.search(r'"sha"[,\:]\s*"([^"]+)"', res).group(1)
            return sign_val, sha_val, hash_val
        except AttributeError:
            return "", "", ""

    def getheader(self, k=0):
        kes = ['ecce0904d756da58b9ea5dd03da3cacea9fa29c6', '7ff70cb6e42cac903d7a4d90c06376fad14080fe57',
               '7f655bc3fc964d76329b7b195b5c51ff42f446e01a']
        headers = {
            'origin': xurl,
            'referer': f'{xurl}/',
            'next-action': kes[k],
